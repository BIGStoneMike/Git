...
{
	if (
		pt.x > r.left && pt.x < r.right					//点のＸ位置が四角の左右の間にいる
		&& pt.y > r.top && pt.y < r.bottom				//点のＹ位置が四角の上下の間にいる
		)
	{
		return TRUE;
	}
...
/// 円と円の当たり判定
/// </summary>
/// <param name="maru1">円の領域１</param>
/// <param name="maru2">円の領域２</param>
/// <returns></returns>
BOOL CheckCollMaruToMaru(MARU maru1, MARU maru2)
{
	...
	float b = maru1.center.x - maru2.center.y;			//三角形の高さの長さを求める
	float c = sqrtf(a * a + b * b);						//三角形の斜辺の長さを求める(sq rt = square root = 平方根)
